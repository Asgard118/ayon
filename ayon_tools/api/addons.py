...
def set_studio_settings(addon_name: str, version: str, settings: dict, url: str, head: str):
	"""
	Обновление конкретной версии аддона, по определенным настройкам
	"""
	response = requests.post(url=f'{url}/api/addons/{addon_name}/{version}/settings', headers=head,
							 json=settings)
...
def set_project_settings(addon_name: str, version: str, project_name: str, settings: dict, url: str, head: str):
	"""
	Обновляет конкретную версию аддона проекта
	"""
	response = requests.post(url=f'{url}/api/addons/{addon_name}/{version}/settings/{project_name}',
							 headers=head, json=settings)
	response.raise_for_status()
